<div class="quiz-result-page">
    ...
    <div class="card result-dashboard">
        <div class="result-score-highlight"><span class="result-score-value"><?= $result['score']===null?'—':e((string)$result['score']).'%'?></span><span class="result-score-label">Objective score · MCQ and Match only</span></div>
...
<?php if($q['type']==='mcq'):?><p>Your answer: <?=e($student!==''?$student:'Unanswered')?></p><p>Correct answer: <?=e((string)($data['correct_answer']??'Unavailable'))?></p><p>Status: <strong><?= $student===''?'Unanswered':((int)$q['is_correct']===1?'Correct':'Incorrect')?></strong></p>
