...
class JsonImporter {
    ...
    private const TYPES=['mcq','complete','match','compare','essay','true_false'];
    ...
    private static function normalize(mixed $record,int $number):array{$errors=[];$warnings=[];if(!is_array($record))return ['number'=>$number,'errors'=>['Question must be an object.'],'warnings'=>[]];$type=$record['type']??null;$question=$record['question']??null;$frequency=$record['frequency']??1;if(!is_string($type)||!in_array($type,self::TYPES,true))$errors[]='type must be one of mcq, complete, match, compare, essay, true_false.';if(!is_string($question)||trim($question)==='')$errors[]='question is required and cannot be empty.';if(!is_int($frequency)||$frequency<1)$errors[]='frequency must be an integer greater than or equal to 1.';$hasAnswer=array_key_exists('answer',$record)&&$record['answer']!==null&&$record['answer']!=='';$answerData=['answer'=>null];
        ...
        elseif($type==='true_false'){if(array_key_exists('answer',$record)&&!is_bool($record['answer'])&&$record['answer']!==null)$errors[]='true_false answer must be a real JSON boolean.';$hasAnswer=is_bool($record['answer']??null);$answerData=['answer'=>$hasAnswer?($record['answer']?'true':'false'):null];}
